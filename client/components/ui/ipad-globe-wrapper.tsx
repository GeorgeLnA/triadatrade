...
export function IpadGlobeWrapper({ className }: IpadGlobeWrapperProps) {
  ...
  // Lock scroll position when iPad completes
  React.useEffect(() => {
    if (ipadComplete && !globeComplete) {
      lockedScrollPositionRef.current = window.scrollY;
    }
  }, [ipadComplete, globeComplete]);
  ...
  React.useEffect(() => {
    if (ipadComplete && !globeComplete) {
      ...
      const preventKeyboardScroll = (e: KeyboardEvent) => {
        if (ipadComplete && !globeComplete) {
          if (['ArrowDown', 'ArrowUp', 'PageDown', 'PageUp', 'Space', 'Home', 'End'].includes(e.key)) {
            ...
            let delta = 0;
            if (e.key === 'ArrowDown' || e.key === 'PageDown') delta = 100;
            else if (e.key === 'ArrowUp' || e.key === 'PageUp') delta = -100;
            else if (e.key === 'Space' && !e.shiftKey) delta = 500;
            else if (e.key === 'Space' && e.shiftKey) delta = -500;
      ...
      const scrollInterval = setInterval(lockScroll, 16); // ~60fps
  ...
  return (
    <motion.div
      style={{
        ...
        top: isLocked ? '50%' : undefined,
        left: isLocked ? '50%' : undefined,
        x: isLocked ? '-50%' : undefined,
        y: isLocked ? '-50%' : undefined,
