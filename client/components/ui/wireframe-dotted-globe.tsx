"use client"

import { useEffect, useRef, useState } from "react"
import * as d3 from "d3"

interface RotatingEarthProps {
  width?: number
  height?: number
  className?: string
}

export default function RotatingEarth({ width = 800, height = 600, className = "" }: RotatingEarthProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const [isLoading, setIsLoading] = useState(true)
  const [error, setError] = useState<string | null>(null)

  useEffect(() => {
    if (!canvasRef.current) return

    const canvas = canvasRef.current
    const context = canvas.getContext("2d")
    if (!context) return

    // Set up responsive dimensions - 1.5x bigger, can extend beyond screen
    const baseSize = Math.min(width, height)
    const maxSize = baseSize * 1.5 // 1.5x bigger
    const containerWidth = maxSize
    const containerHeight = maxSize
    const radius = maxSize / 2.5

    const dpr = window.devicePixelRatio || 1
    canvas.width = containerWidth * dpr
    canvas.height = containerHeight * dpr
    canvas.style.width = `${containerWidth}px`
    canvas.style.height = `${containerHeight}px`
    context.scale(dpr, dpr)

    // Create projection and path generator for Canvas
    const projection = d3
      .geoOrthographic()
      .scale(radius)
      .translate([containerWidth / 2, containerHeight / 2])
      .clipAngle(90)

    const path = d3.geoPath().projection(projection).context(context)

    const pointInPolygon = (point: [number, number], polygon: number[][]): boolean => {
      const [x, y] = point
      let inside = false

      for (let i = 0, j = polygon.length - 1; i < polygon.length; j = i++) {
        const [xi, yi] = polygon[i]
        const [xj, yj] = polygon[j]

        if (yi > y !== yj > y && x < ((xj - xi) * (y - yi)) / (yj - yi) + xi) {
          inside = !inside
        }
      }

      return inside
    }

    const pointInFeature = (point: [number, number], feature: any): boolean => {
      const geometry = feature.geometry

      if (geometry.type === "Polygon") {
        const coordinates = geometry.coordinates
        // Check if point is in outer ring
        if (!pointInPolygon(point, coordinates[0])) {
          return false
        }
        // Check if point is in any hole (inner rings)
        for (let i = 1; i < coordinates.length; i++) {
          if (pointInPolygon(point, coordinates[i])) {
            return false // Point is in a hole
          }
        }
        return true
      } else if (geometry.type === "MultiPolygon") {
        // Check each polygon in the MultiPolygon
        for (const polygon of geometry.coordinates) {
          // Check if point is in outer ring
          if (pointInPolygon(point, polygon[0])) {
            // Check if point is in any hole
            let inHole = false
            for (let i = 1; i < polygon.length; i++) {
              if (pointInPolygon(point, polygon[i])) {
                inHole = true
                break
              }
            }
            if (!inHole) {
              return true
            }
          }
        }
        return false
      }

      return false
    }

    const generateDotsInPolygon = (feature: any, dotSpacing = 16) => {
      const dots: [number, number][] = []
      const bounds = d3.geoBounds(feature)
      const [[minLng, minLat], [maxLng, maxLat]] = bounds

      const stepSize = dotSpacing * 0.08
      let pointsGenerated = 0

      for (let lng = minLng; lng <= maxLng; lng += stepSize) {
        for (let lat = minLat; lat <= maxLng; lat += stepSize) {
          const point: [number, number] = [lng, lat]
          if (pointInFeature(point, feature)) {
            dots.push(point)
            pointsGenerated++
          }
        }
      }

      return dots
    }

    interface DotData {
      lng: number
      lat: number
      visible: boolean
    }

    const allDots: DotData[] = []
    let landFeatures: any

    const render = () => {
      // Clear canvas
      context.clearRect(0, 0, containerWidth, containerHeight)

      const currentScale = projection.scale()
      const scaleFactor = currentScale / radius

      // Draw ocean (globe background)
      context.beginPath()
      context.arc(containerWidth / 2, containerHeight / 2, currentScale, 0, 2 * Math.PI)
      context.fillStyle = "#000000"
      context.fill()
      context.strokeStyle = "#ffffff"
      context.lineWidth = 2 * scaleFactor
      context.stroke()

      // Create invisible elevated globe layer for 3D flight paths
      const elevatedScale = currentScale * 1.15 // 15% larger than main globe
      const elevatedProjection = d3
        .geoOrthographic()
        .scale(elevatedScale)
        .translate([containerWidth / 2, containerHeight / 2])
        .clipAngle(90)
        .rotate(rotation)

      if (landFeatures) {
        // Draw graticule
        const graticule = d3.geoGraticule()
        context.beginPath()
        path(graticule())
        context.strokeStyle = "#ffffff"
        context.lineWidth = 1 * scaleFactor
        context.globalAlpha = 0.25
        context.stroke()
        context.globalAlpha = 1

        // Draw land outlines
        context.beginPath()
        landFeatures.features.forEach((feature: any) => {
          path(feature)
        })
        context.strokeStyle = "#ffffff"
        context.lineWidth = 1 * scaleFactor
        context.stroke()

        // Draw Ukraine with special highlighting using actual outline
        if (landFeatures.ukraine) {
          console.log("Drawing Ukraine outline:", landFeatures.ukraine.properties)
          // Draw Ukraine outline with green highlight
          context.shadowColor = "#00ff00"
          context.shadowBlur = 15 * scaleFactor
          context.beginPath()
          path(landFeatures.ukraine)
          context.strokeStyle = "#00ff00"
          context.lineWidth = 3 * scaleFactor
          context.stroke()
          
          // Reset shadow
          context.shadowColor = "transparent"
          context.shadowBlur = 0
          
          // Draw inner outline for better visibility
          context.beginPath()
          path(landFeatures.ukraine)
          context.strokeStyle = "#ffffff"
          context.lineWidth = 1.5 * scaleFactor
          context.stroke()
        } else {
          // Manual fallback: Draw Ukraine outline using REAL coordinates
          const ukraineCoords = [
            [31.785998, 52.101678], [32.159412, 52.061267], [32.412058, 52.288695], [32.715761, 52.238465], [33.7527, 52.335075], 
            [34.391731, 51.768882], [34.141978, 51.566413], [34.224816, 51.255993], [35.022183, 51.207572], [35.377924, 50.773955], 
            [35.356116, 50.577197], [36.626168, 50.225591], [37.39346, 50.383953], [38.010631, 49.915662], [38.594988, 49.926462], 
            [40.069058, 49.601055], [40.080789, 49.30743], [39.674664, 48.783818], [39.895632, 48.232405], [39.738278, 47.898937], 
            [38.770585, 47.825608], [38.255112, 47.5464], [38.223538, 47.10219], [37.425137, 47.022221], [36.759855, 46.6987], 
            [35.823685, 46.645964], [34.962342, 46.273197], [35.020788, 45.651219], [35.510009, 45.409993], [36.529998, 45.46999], 
            [36.334713, 45.113216], [35.239999, 44.939996], [33.882511, 44.361479], [33.326421, 44.564877], [33.546924, 45.034771], 
            [32.454174, 45.327466], [32.630804, 45.519186], [33.588162, 45.851569], [33.298567, 46.080598], [31.74414, 46.333348], 
            [31.675307, 46.706245], [30.748749, 46.5831], [30.377609, 46.03241], [29.603289, 45.293308], [29.149725, 45.464925], 
            [28.679779, 45.304031], [28.233554, 45.488283], [28.485269, 45.596907], [28.659987, 45.939987], [28.933717, 46.25883], 
            [28.862972, 46.437889], [29.072107, 46.517678], [29.170654, 46.379262], [29.759972, 46.349988], [30.024659, 46.423937], 
            [29.83821, 46.525326], [29.908852, 46.674361], [29.559674, 46.928583], [29.415135, 47.346645], [29.050868, 47.510227], 
            [29.122698, 47.849095], [28.670891, 48.118149], [28.259547, 48.155562], [27.522537, 48.467119], [26.857824, 48.368211], 
            [26.619337, 48.220726], [26.19745, 48.220881], [25.945941, 47.987149], [25.207743, 47.891056], [24.866317, 47.737526], 
            [24.402056, 47.981878], [23.760958, 47.985598], [23.142236, 48.096341], [22.710531, 47.882194], [22.64082, 48.15024], 
            [22.085608, 48.422264], [22.280842, 48.825392], [22.558138, 49.085738], [22.776419, 49.027395], [22.51845, 49.476774], 
            [23.426508, 50.308506], [23.922757, 50.424881], [24.029986, 50.705407], [23.527071, 51.578454], [24.005078, 51.617444], 
            [24.553106, 51.888461], [25.327788, 51.910656], [26.337959, 51.832289], [27.454066, 51.592303], [28.241615, 51.572227], 
            [28.617613, 51.427714], [28.992835, 51.602044], [29.254938, 51.368234], [30.157364, 51.416138], [30.555117, 51.319503], 
            [30.619454, 51.822806], [30.927549, 52.042353], [31.785998, 52.101678]
          ]
          
          // Draw manual Ukraine outline
          context.shadowColor = "#00ff00"
          context.shadowBlur = 15 * scaleFactor
          context.beginPath()
          
          ukraineCoords.forEach((coord, index) => {
            const projected = projection(coord)
            if (projected) {
              if (index === 0) {
                context.moveTo(projected[0], projected[1])
              } else {
                context.lineTo(projected[0], projected[1])
              }
            }
          })
          context.closePath()
          
          context.strokeStyle = "#00ff00"
          context.lineWidth = 3 * scaleFactor
          context.stroke()
          
          // Reset shadow
          context.shadowColor = "transparent"
          context.shadowBlur = 0
          
          // Draw inner outline
          context.beginPath()
          ukraineCoords.forEach((coord, index) => {
            const projected = projection(coord)
            if (projected) {
              if (index === 0) {
                context.moveTo(projected[0], projected[1])
              } else {
                context.lineTo(projected[0], projected[1])
              }
            }
          })
          context.closePath()
          
          context.strokeStyle = "#ffffff"
          context.lineWidth = 1.5 * scaleFactor
          context.stroke()
          
          // Draw curved flight path lines to UK, Turkey, and US
          const ukraineCenter = [31.5, 49.0] // Ukraine center coordinates
          const ukCenter = [-0.1, 51.5] // London, UK coordinates
          const turkeyCenter = [35.2, 39.9] // Ankara, Turkey coordinates
          const usCenter = [-74.0, 40.7] // New York, US coordinates
          
          // Function to create elevated curved path that goes above globe surface
          const createCurvedPath = (start: [number, number], end: [number, number]) => {
            const path: [number, number][] = []
            const steps = 50
            const elevationHeight = 0.3 // How high above globe surface (0.0 = on surface, 1.0 = very high)
            
            for (let i = 0; i <= steps; i++) {
              const t = i / steps
              
              // Calculate midpoint for the curve
              const lat1 = start[1] * Math.PI / 180
              const lng1 = start[0] * Math.PI / 180
              const lat2 = end[1] * Math.PI / 180
              const lng2 = end[0] * Math.PI / 180
              
              // Great circle distance
              const d = Math.acos(Math.sin(lat1) * Math.sin(lat2) + Math.cos(lat1) * Math.cos(lat2) * Math.cos(lng2 - lng1))
              
              if (d === 0) continue
              
              // Calculate normal great circle path
              const a = Math.sin((1 - t) * d) / Math.sin(d)
              const b = Math.sin(t * d) / Math.sin(d)
              
              let x = a * Math.cos(lat1) * Math.cos(lng1) + b * Math.cos(lat2) * Math.cos(lng2)
              let y = a * Math.cos(lat1) * Math.sin(lng1) + b * Math.cos(lat2) * Math.sin(lng2)
              let z = a * Math.sin(lat1) + b * Math.sin(lat2)
              
              // Add elevation by pushing points outward from center (above globe surface)
              const elevation = Math.sin(t * Math.PI) * elevationHeight // Peak at midpoint
              x *= (1 + elevation)
              y *= (1 + elevation)
              z *= (1 + elevation)
              
              // Convert back to lat/lng
              const lat = Math.atan2(z, Math.sqrt(x * x + y * y)) * 180 / Math.PI
              const lng = Math.atan2(y, x) * 180 / Math.PI
              
              path.push([lng, lat])
            }
            
            return path
          }
          
          // Draw smooth arching line to UK (layer 1 -> layer 2 -> layer 1)
          const ukPath = createCurvedPath(ukraineCenter, ukCenter)
          context.shadowColor = "#00ff00"
          context.shadowBlur = 10 * scaleFactor
          context.beginPath()
          ukPath.forEach((point, index) => {
            const t = index / (ukPath.length - 1)
            
            // Smooth interpolation between globe layers using sine wave
            const archHeight = Math.sin(t * Math.PI) // 0 at start/end, 1 at middle
            const mainProjected = projection(point)
            const elevatedProjected = elevatedProjection(point)
            
            if (mainProjected && elevatedProjected) {
              // Smooth interpolation between the two projections
              const x = mainProjected[0] + (elevatedProjected[0] - mainProjected[0]) * archHeight
              const y = mainProjected[1] + (elevatedProjected[1] - mainProjected[1]) * archHeight
              
              if (index === 0) {
                context.moveTo(x, y)
              } else {
                context.lineTo(x, y)
              }
            }
          })
          context.strokeStyle = "#00ff00"
          context.lineWidth = 1.5 * scaleFactor
          context.globalAlpha = 0.8
          context.stroke()
          
          // Reset shadow for next line
          context.shadowColor = "transparent"
          context.shadowBlur = 0
          
          // Draw smooth arching line to Turkey (layer 1 -> layer 2 -> layer 1)
          const turkeyPath = createCurvedPath(ukraineCenter, turkeyCenter)
          context.shadowColor = "#00ff00"
          context.shadowBlur = 10 * scaleFactor
          context.beginPath()
          turkeyPath.forEach((point, index) => {
            const t = index / (turkeyPath.length - 1)
            
            // Smooth interpolation between globe layers using sine wave
            const archHeight = Math.sin(t * Math.PI) // 0 at start/end, 1 at middle
            const mainProjected = projection(point)
            const elevatedProjected = elevatedProjection(point)
            
            if (mainProjected && elevatedProjected) {
              // Smooth interpolation between the two projections
              const x = mainProjected[0] + (elevatedProjected[0] - mainProjected[0]) * archHeight
              const y = mainProjected[1] + (elevatedProjected[1] - mainProjected[1]) * archHeight
              
              if (index === 0) {
                context.moveTo(x, y)
              } else {
                context.lineTo(x, y)
              }
            }
          })
          context.strokeStyle = "#00ff00"
          context.lineWidth = 1.5 * scaleFactor
          context.globalAlpha = 0.8
          context.stroke()
          
          // Reset shadow for next line
          context.shadowColor = "transparent"
          context.shadowBlur = 0
          
          // Draw smooth arching line to US (layer 1 -> layer 2 -> layer 1)
          const usPath = createCurvedPath(ukraineCenter, usCenter)
          context.shadowColor = "#00ff00"
          context.shadowBlur = 10 * scaleFactor
          context.beginPath()
          usPath.forEach((point, index) => {
            const t = index / (usPath.length - 1)
            
            // Smooth interpolation between globe layers using sine wave
            const archHeight = Math.sin(t * Math.PI) // 0 at start/end, 1 at middle
            const mainProjected = projection(point)
            const elevatedProjected = elevatedProjection(point)
            
            if (mainProjected && elevatedProjected) {
              // Smooth interpolation between the two projections
              const x = mainProjected[0] + (elevatedProjected[0] - mainProjected[0]) * archHeight
              const y = mainProjected[1] + (elevatedProjected[1] - mainProjected[1]) * archHeight
              
              if (index === 0) {
                context.moveTo(x, y)
              } else {
                context.lineTo(x, y)
              }
            }
          })
          context.strokeStyle = "#00ff00"
          context.lineWidth = 1.5 * scaleFactor
          context.globalAlpha = 0.8
          context.stroke()
          
          // Reset shadow
          context.shadowColor = "transparent"
          context.shadowBlur = 0
          
          // Draw landing markers with city names
          const landingPoints = [
            { coords: ukraineCenter, name: "KYIV", country: "UKRAINE", isStart: true },
            { coords: ukCenter, name: "LONDON", country: "UK" },
            { coords: turkeyCenter, name: "ANKARA", country: "TURKEY" },
            { coords: usCenter, name: "NEW YORK", country: "USA" }
          ]
          
          landingPoints.forEach(point => {
            const projected = projection(point.coords)
            if (projected) {
              // Draw landing marker pin (larger for starting point)
              const markerSize = point.isStart ? 8 * scaleFactor : 6 * scaleFactor
              const shadowSize = point.isStart ? 12 * scaleFactor : 8 * scaleFactor
              
              context.beginPath()
              context.arc(projected[0], projected[1], markerSize, 0, 2 * Math.PI)
              context.fillStyle = "#00ff00"
              context.shadowColor = "#00ff00"
              context.shadowBlur = shadowSize
              context.fill()
              
              // Draw marker outline (thicker for starting point)
              context.beginPath()
              context.arc(projected[0], projected[1], markerSize, 0, 2 * Math.PI)
              context.strokeStyle = "#ffffff"
              context.lineWidth = point.isStart ? 3 * scaleFactor : 2 * scaleFactor
              context.shadowColor = "transparent"
              context.shadowBlur = 0
              context.stroke()
              
              // Draw city name label (larger for starting point)
              const fontSize = point.isStart ? 14 * scaleFactor : 12 * scaleFactor
              context.fillStyle = "#ffffff"
              context.font = `bold ${fontSize}px Arial`
              context.textAlign = "center"
              context.textBaseline = "top"
              
              // Add background for text readability
              const textMetrics = context.measureText(point.name)
              const textWidth = textMetrics.width
              const textHeight = fontSize
              
              context.fillStyle = "rgba(0, 0, 0, 0.7)"
              context.fillRect(
                projected[0] - textWidth/2 - 4 * scaleFactor,
                projected[1] + (point.isStart ? 14 : 12) * scaleFactor - 2 * scaleFactor,
                textWidth + 8 * scaleFactor,
                textHeight + 4 * scaleFactor
              )
              
              // Draw text (different color for starting point)
              context.fillStyle = point.isStart ? "#ffff00" : "#00ff00" // Yellow for Kyiv, green for others
              context.fillText(point.name, projected[0], projected[1] + (point.isStart ? 14 : 12) * scaleFactor)
              
              // Draw country name below
              context.font = `${10 * scaleFactor}px Arial`
              context.fillStyle = "#ffffff"
              context.fillText(point.country, projected[0], projected[1] + (point.isStart ? 32 : 28) * scaleFactor)
            }
          })
          
          // Reset alpha
          context.globalAlpha = 1
        }

        // Halftone dots removed for cleaner look
      }
    }

    const loadWorldData = async () => {
      try {
        setIsLoading(true)

        const response = await fetch(
          "https://raw.githubusercontent.com/martynafford/natural-earth-geojson/refs/heads/master/110m/physical/ne_110m_land.json",
        )
        if (!response.ok) throw new Error("Failed to load land data")

        landFeatures = await response.json()

        // Load Ukraine data separately - try multiple sources
        try {
          // Try Natural Earth countries data first
          const countriesResponse = await fetch(
          "https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson"
        )
          if (countriesResponse.ok) {
            const worldData = await countriesResponse.json()
            console.log("World data loaded, searching for Ukraine...")
            
          // Find Ukraine in the world data - try multiple property names
          const ukraine = worldData.features.find((feature: any) => {
            const props = feature.properties || {}
            return props.NAME === "Ukraine" || 
                   props.ADMIN === "Ukraine" ||
                   props.NAME_EN === "Ukraine" ||
                   props.NAME_DE === "Ukraine" ||
                   props.NAME_ES === "Ukraine" ||
                   props.NAME_FR === "Ukraine" ||
                   (props.NAME && props.NAME.toLowerCase().includes("ukraine")) ||
                   (props.ADMIN && props.ADMIN.toLowerCase().includes("ukraine"))
          })
            
          if (ukraine) {
            landFeatures.ukraine = ukraine
              console.log("Ukraine data loaded from world data:", ukraine.properties)
            } else {
              console.log("Ukraine not found in world data. Available countries:", worldData.features.map(f => f.properties?.NAME || f.properties?.ADMIN).slice(0, 20))
              
              // Try alternative search
              const ukraineAlt = worldData.features.find((feature: any) => {
                const props = feature.properties || {}
                const name = (props.NAME || props.ADMIN || "").toLowerCase()
                return name.includes("ukraine") || name.includes("ukraina")
              })
              if (ukraineAlt) {
                landFeatures.ukraine = ukraineAlt
                console.log("Ukraine found with alternative search:", ukraineAlt.properties)
          } else {
                // Try Natural Earth admin countries as fallback
                try {
                  const adminResponse = await fetch(
                    "https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson"
                  )
                  if (adminResponse.ok) {
                    const adminData = await adminResponse.json()
                    const ukraineAdmin = adminData.features.find((feature: any) => {
                      const props = feature.properties || {}
                      return (props.ADMIN && props.ADMIN.toLowerCase().includes("ukraine")) ||
                             (props.NAME && props.NAME.toLowerCase().includes("ukraine"))
                    })
                    if (ukraineAdmin) {
                      landFeatures.ukraine = ukraineAdmin
                      console.log("Ukraine found in admin data:", ukraineAdmin.properties)
                    }
                  }
                } catch (adminError) {
                  console.log("Error loading admin data:", adminError)
                }
              }
            }
          }
        } catch (error) {
          console.log("Error loading Ukraine data:", error)
        }

        // Generate dots for all land features
        let totalDots = 0
        landFeatures.features.forEach((feature: any) => {
          const dots = generateDotsInPolygon(feature, 16)
          dots.forEach(([lng, lat]) => {
            allDots.push({ lng, lat, visible: true })
            totalDots++
          })
        })


        render()
        setIsLoading(false)
      } catch (err) {
        setError("Failed to load land map data")
        setIsLoading(false)
      }
    }

    // Set up rotation and interaction
    const rotation = [0, 0]
    let autoRotate = true
    const rotationSpeed = 0.1

    const rotate = () => {
      if (autoRotate) {
        rotation[0] += rotationSpeed
        projection.rotate(rotation)
        render()
      }
    }

    // Auto-rotation timer
    const rotationTimer = d3.timer(rotate)

    const handleMouseDown = (event: MouseEvent) => {
      autoRotate = false
      const startX = event.clientX
      const startY = event.clientY
      const startRotation = [...rotation]

      const handleMouseMove = (moveEvent: MouseEvent) => {
        const sensitivity = 0.5
        const dx = moveEvent.clientX - startX
        const dy = moveEvent.clientY - startY

        rotation[0] = startRotation[0] + dx * sensitivity
        rotation[1] = startRotation[1] - dy * sensitivity
        rotation[1] = Math.max(-90, Math.min(90, rotation[1]))

        projection.rotate(rotation)
        render()
      }

      const handleMouseUp = () => {
        document.removeEventListener("mousemove", handleMouseMove)
        document.removeEventListener("mouseup", handleMouseUp)

        setTimeout(() => {
          autoRotate = true
        }, 10)
      }

      document.addEventListener("mousemove", handleMouseMove)
      document.addEventListener("mouseup", handleMouseUp)
    }

    canvas.addEventListener("mousedown", handleMouseDown)

    // Load the world data
    loadWorldData()

    // Cleanup
    return () => {
      rotationTimer.stop()
      canvas.removeEventListener("mousedown", handleMouseDown)
    }
  }, [width, height])

  if (error) {
    return (
      <div className={`dark flex items-center justify-center bg-card rounded-2xl p-8 ${className}`}>
        <div className="text-center">
          <p className="dark text-destructive font-semibold mb-2">Error loading Earth visualization</p>
          <p className="dark text-muted-foreground text-sm">{error}</p>
        </div>
      </div>
    )
  }

  return (
    <div className={`relative flex items-center justify-start ${className}`} style={{ overflow: 'visible' }}>
      <canvas
        ref={canvasRef}
        className="rounded-2xl bg-transparent"
        style={{ 
          width: "100%",
          height: "auto",
          maxWidth: "none",
          aspectRatio: "1 / 1",
          marginLeft: "-40%"
        }}
      />
    </div>
  )
}
