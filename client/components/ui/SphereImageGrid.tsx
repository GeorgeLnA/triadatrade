...
export interface SphereImageGridProps {
  ...
  centerOverlayStyle?: CSSProperties;
  centerOverlayZIndex?: number;
}
...
const SPHERE_MATH = {
  ...
  calculateDistance: (pos: Position3D, center: Position3D = { x: 0, y: 0, z: 0 }) => {
...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const generateSpherePositions = useCallback((): SphericalPosition[] => {
    ...
    for (let i = 0; i < imageCount; i++) {
      ...
      const jitterTheta = (Math.random() - 0.5) * 4;
      const jitterPhi = (Math.random() - 0.5) * 3;
      const adjustedTheta = (theta + jitterTheta + 360) % 360;
      const adjustedPhi = Math.max(5, Math.min(175, phi + jitterPhi));
  ...
  const calculateWorldPositions = useCallback((): WorldPosition[] => {
    const positions = imagePositions.map((pos, index) => {
      ...
      const depthRatio = (worldPos.z + actualSphereRadius) / (2 * actualSphereRadius);
      const fadeOpacity = Math.max(0.35, Math.min(1, depthRatio + 0.4));
    ...
    for (let i = 0; i < adjustedPositions.length; i++) {
      ...
      for (let j = 0; j < adjustedPositions.length; j++) {
        ...
        const minDistance = (imageSize + otherSize) / 2 + 18;

        if (distance < minDistance && distance > 0) {
          const overlap = minDistance - distance;
          const reductionFactor = Math.max(0.65, 1 - (overlap / minDistance) * 0.35);
          const targetScale = adjustedScale * reductionFactor;
          adjustedScale = Math.max(Math.min(adjustedScale, targetScale), adjustedScale * 0.85);
        }
      }

      adjustedPositions[i] = {
        ...pos,
        scale: Math.max(0.35, adjustedScale)
      };
    }
  ...
  const renderSpotlightModal = () => {
    ...
    const modalTitle = selectedImage.title ?? selectedImage.alt;
    const modalDescription = selectedImage.description ?? selectedImage.alt;
    ...
    <>
      ...
      >
        <div className="relative h-full w-full">
          {centerOverlay && (
            <div
              className={`pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 ${centerOverlayClassName ?? ""}`}
              style={{
                zIndex: centerOverlayZIndex,
                ...centerOverlayStyle
              }}
            >
              {centerOverlay}
